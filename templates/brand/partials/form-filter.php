    ...
    <div class="form px-8 py-6 shadow-lg mb-12 rounded-lg bg-white">
        <div class="text-2xl font-[500] mb-6 font-[Monsterat]">
            <?php /* echo esc_html(sprintf(
                esc_html__('Schedule %s', 'dvutheme'),
                $term->name
            ))  */

            echo esc_html(esc_html__('Schedule', 'dvutheme'));
            ?>
        </div>
        <form id="activity_post_search_form" method="POST" class="">
            ...
            <div class="flex flex-wrap -mx-3 gap-y-6">
                <div class="item px-3">
                    <legend class="mb-3 font-[500] block"><?php esc_html_e("Event Localtion", 'dvutheme') ?></legend>
                ...
                <div class="item px-3">
                    <legend class="mb-3 font-[500] block"><?php esc_html_e("Event Phase", 'dvutheme') ?></legend>
                    <div class="flex items-center -mx-1">
                        <div class="checkbox-control px-1">
                            <input type="checkbox" id="activity-before_event" name="before_event" class="hidden" />
                            <label for="activity-before_event" class="border rounded-full px-4 py-2 block cursor-pointer h-10"><?php esc_html_e("Pre-event", 'dvutheme') ?></label>
                        </div>
                        <div class="checkbox-control px-1">
                            <input type="checkbox" id="activity-in_event" name="in_event" class="hidden" />
                            <label for="activity-in_event" class="border rounded-full px-4 py-2 block cursor-pointer h-10"><?php esc_html_e("During event", 'dvutheme') ?></label>
                        </div>
                        <div class="checkbox-control px-1">
                            <input type="checkbox" id="activity-after_event" name="after_event" class="hidden" />
                            <label for="activity-after_event" class="border rounded-full px-4 py-2 block cursor-pointer h-10"><?php esc_html_e("Post-event", 'dvutheme') ?></label>
                        </div>
                    </div>
                </div>
                <div class="item px-3">
